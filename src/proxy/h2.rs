...
pub mod client {
    use super::{H2Stream, H2StreamReadHalf, H2StreamWriteHalf, SendBuf};
    use crate::config;
...
pub mod server {
    use super::{do_ping_pong, H2Stream, H2StreamReadHalf, H2StreamWriteHalf, SendBuf};
    use crate::config;
    use crate::proxy::Error;
    ...
    pub struct H2Request {
        request: Parts,
        recv: h2::RecvStream,
        send: h2::server::SendResponse<SendBuf>,
    }
    ...
    impl H2Request {
        ...
        pub async fn send_response(self, resp: Response<()>) -> Result<H2Stream, Error> {
            let H2Request { recv, mut send, .. } = self;
            let send = send.send_response(resp, false)?;
            let dropped = Arc::new(());
            let stream_count = Arc::new(AtomicU16::new(0));
            let read = H2StreamReadHalf {
                recv_stream: recv,
                buf: Default::default(),
                half_dropped: dropped.clone(),
                active_count: stream_count.clone(),
            };
            let write = H2StreamWriteHalf {
                send_stream: send,
                half_dropped: dropped,
