...
struct PoolEntry {
    ...
    notify: Notify,
    mapped: AtomicBool,
    // These counters only govern entry lifetime. Relaxed ordering is sufficient because the
    // mapped tombstone prevents new users while cleanup rechecks every counter before removal.
    users: AtomicUsize,
    waiters: AtomicUsize,
...
impl PoolEntry {
    ...
    fn remove_user(&self) -> bool {
        self.users.fetch_sub(1, Ordering::Relaxed) == 1
            ...
            && matches!(self.inner.lock().unwrap().state, EntryState::Empty)
    }
    ...
    fn next_action(self: &Arc<Self>) -> EntryAction {
        ...
        match inner.state {
            ...
            EntryState::Connecting { .. } => self.wait_action(),
...
impl PoolState {
    ...
    fn cleanup_entry(&self, expected: &Arc<PoolEntry>) {
        ...
        if !expected.is_idle() {
            expected.mapped.store(true, Ordering::Release);
            return;
        }
...
impl WorkloadHBONEPool {
    ...
    pub async fn send_request_pooled(
        ...
        let (mut stream, baggage) = match connection.send_request(request).await {
            Ok(result) => result,
            Err(error) => {
                self.state.recover_capacity(connection);
                return Err(error);
            }
        };
...
mod test {
    ...
    async fn pool_idle_expiration_is_generation_safe() {
        ...
        let entry = pool
            ...
            .unwrap();
        drop(guard);
        assert_eq!(entry.connections.load(Ordering::Relaxed), 1);
    ...
    async fn pool_randomized_lifecycle_stress_preserves_entry_invariants() {
        ...
        for (_, entry) in pool.state.entries.iter(&guard) {
            ...
            assert_eq!(entry.waiters.load(Ordering::Relaxed), 0);
        }
        drop(guard);
