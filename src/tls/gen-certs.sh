...
if [ ! -f ca-key.pem ]; then
    ...
    openssl req -x509 -new -nodes -addext "keyUsage = keyCertSign, cRLSign" -key "ca-key.pem" -days 100000 -out "root-cert.pem" -subj "/O=cluster.local"
fi
if [ ! -f ca-key2.pem ]; then
    # Only gen if doesn't exist. As some tests depend on the existing content of root cert.
    openssl genrsa -f4 -out ca-key2.pem
    openssl req -x509 -new -nodes -addext "keyUsage = keyCertSign, cRLSign" -key "ca-key2.pem" -days 100000 -out "root-cert2.pem" -subj "/O=cluster.local"
fi
